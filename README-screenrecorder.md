...
Bu depoya `screen_recorder.py` adlı basit bir ekran kayıt aracı eklendi.
...
```bash
python screen_recorder.py --output kayit.mp4 --fps 60 --duration 10
```

İkinci monitörü kaydetme:

```bash
python screen_recorder.py --monitor 2
```
...
```text
Kayıt başladı: C:\Users\Kullanici\Desktop\recording_20260101_120000.mp4
...
Video dosyası: C:\Users\Kullanici\Desktop\recording_20260101_120000.mp4
```

İstersen doğrudan Masaüstüne kaydetmek için şöyle çalıştırabilirsin:

```bash
python screen_recorder.py --output "%USERPROFILE%\Desktop\kayit.mp4"
```
